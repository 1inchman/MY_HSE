...
class main(QtGui.QDialog):
    def __init__(self, parent=None):
        ...
        self.buttnames = [u'Отчет №1: кол-ва обр учр-ий и парков по районам',
                          ...
                          u'Отчет №6: районы с необходимым количеством учр-ий']
        self.buttons = []
        ...
        text = [u'Количество рынков', u'Количество парков', u'Количество обр учр-ий', u'Количество баров']
        self.labels = []
        self.texteds = []
        for tx in text:
        ...
        QtCore.QObject.connect(self.buttons[8], QtCore.SIGNAL("clicked()"), self.query6)
...
"ORDER BY distance, distName"
    ...
    def query6(self):
        ...
        model = QtSql.QSqlRelationalTableModel()
        query = QtSql.QSqlQuery()
        query.prepare("SELECT DISTINCT t1.distname AS district "
"FROM district t1 "
"INNER JOIN eduplace t2 ON (t1.id=t2.distid) "
"INNER JOIN park t3 ON (t1.id=t3.distid) "
"INNER JOIN market t4 ON (t1.id=t4.distid) "
"INNER JOIN bar t5 ON (t1.id=t5.distid) "
"GROUP BY t1.distname "
"HAVING count(distinct t2.id)>=:edu and count(distinct t3.id)>=:park and count(distinct t4.id)>=:market and count(distinct t5.id)>=:bar")
        query.bindValue(':edu', row[2])
        query.bindValue(':park', row[1])
        query.bindValue(':market', row[0])
        query.bindValue(':bar', row[3])
        query.exec_()
        print query.lastError().text()
        model.setQuery(query)
        self.table.setModel(model)
